...
inherit python-any-r1 cmake-multilib versionator
...
multilib_src_install() {
	...
	cd "${BUILD_DIR}/loader"
	dolib libvulkan.so.${MY_PV}
	dosym libvulkan.so.${MY_PV} /usr/$(get_libdir)/libvulkan.so.1
	dosym libvulkan.so.${MY_PV} /usr/$(get_libdir)/libvulkan.so
