...
BDEPEND="
	...
	vulkan? (
		video_cards_amdgpu? ( dev-util/glslang )
		video_cards_radeonsi? ( dev-util/glslang )
	)
...
llvm_check_deps() {
	...
	if use video_cards_r600 || use video_cards_radeon || use video_cards_radeonsi || use video_cards_amdgpu
	then
...
pkg_pretend() {
	if use vulkan; then
		if ! use video_cards_d3d12 &&
		   ...
		   ! use video_cards_v3d; then
			ewarn "Ignoring USE=vulkan     since VIDEO_CARDS does not contain d3d12, freedreno, intel, amdgpu, radeonsi, or v3d"
		fi
	fi
	...
	if use vaapi; then
		if ! use video_cards_d3d12 &&
		   ...
		   ! use video_cards_nouveau; then
			ewarn "Ignoring USE=vaapi      since VIDEO_CARDS does not contain d3d12, r600, amdgpu, radeonsi, or nouveau"
		fi
	fi
	...
	if use vdpau; then
		if ! use video_cards_d3d12 &&
		   ...
		   ! use video_cards_radeonsi &&
		   ! use video_cards_nouveau; then
			ewarn "Ignoring USE=vdpau      since VIDEO_CARDS does not contain d3d12, r300, r600, amdgpu, radeonsi, or nouveau"
		fi
	fi
...
multilib_src_configure() {
	...
	gallium_enable video_cards_amdgpu amdgpu
