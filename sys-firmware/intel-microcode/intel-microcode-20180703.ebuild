...
SLOT="0"
KEYWORDS="-* amd64 x86"
IUSE="hostonly initramfs +split-ucode vanilla split-usr"
...
pkg_postinst() {
	...
	if use initramfs && [[ -s "${EROOT%/}/boot/intel-uc.img" ]]; then
		...
		_has_installed_something=$(find "${EROOT%/}/usr/lib/firmware/intel-ucode" -maxdepth 0 -not -empty -exec echo yes \;)
	fi
